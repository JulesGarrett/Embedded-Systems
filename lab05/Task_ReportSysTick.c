...
static uint32_t Task_ReportADC_Status = 0;
...
//*****************************************************************************
	...
	while(1){
		...
		char 	TimeString2[24];
				...
				Task_ReportSysTick_Status = sprintf( TimeString, "Task_SysTick: %d", SysTickCount_Nbr );
				Task_ReportSysTick_Status = Task_ReportSysTick_Status;

				Task_ReportADC_Status = sprintf( TimeString2, "Task_ADC_Sample: %04d", ulValue );
				Task_ReportADC_Status = Task_ReportADC_Status;

				RIT128x96x4StringDraw( TimeString, 0, 12, 15);
			    RIT128x96x4StringDraw( TimeString2, 0, 24, 15);
			    vTaskDelay( 10000 );
